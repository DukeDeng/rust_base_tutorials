...
fn about_number() {
    ...
    println!("d: {:?}", d); // (19, true) 代表溢出了 19

    let e = a.saturating_add(200);
    println!("e: {}", e); // 255 最大值

    assert!((0.1_f64 +  0.2 - 0.3).abs() < 0.001); 
    // 在Rust 中使用abs()函数获取绝对值 要获得绝对值,我们需要明确指定数据类型。
    // 对 f32 类型做加法时，0.1 + 0.2 的结果是 3e99999a，0.3 也是 3e99999a，因此 f32 下的 0.1 + 0.2 == 0.3 通过测试
    // 但是到了 f64 类型时，结果就不一样了，因为 f64 精度高很多，因此在小数点非常后面发生了一点微小的变化，0.1 + 0.2 以 4 结尾，但是 0.3 以3结尾// 浮点数精度问题

    assert!(0.1_f32 + 0.2_f32 == 0.3_f32);
    for c in 'a'..='z' {
        println!("{}", c as u8); // 输出：97 到 122 as 强转化类型
    }
}
...
fn about_char_bool() {
    ...
    let y = '中';
    let z = '你';
    assert_eq!(size_of_val(&x), 4); // char 类型大小为 4    
    assert_eq!(size_of_val(&y), 4); // char 类型大小为 4    
    assert_eq!(size_of_val(&z), 4); // char 类型大小为 4    
}

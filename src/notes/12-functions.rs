...
impl Rectangle {
    ...
    // 该方法的调用者是可变的，`&mut self` 是 `self: &mut Self` 的语法糖
...
fn  main () {
    ...
    // 所以，我们可以直接调用 `rectangle.perimeter()` 来获取面积: `rectangle.perimeter()` = `Rectangle::perimeter(&rectangle)`
    ...
    // 正确的做法是使用可变借用 `&mut square` 来调用 `translate` 方法
